...
if OUTPUT_DOCUMENT_TYPE == "html":
    ...
    m = max(len(skill_group) for skill_group in skills_db['skill_groups'])

    table = document.add_table(rows=m, cols=len(skills_db['skill_groups']))
    col = 0
    ...
    for experience in skills_db['education']:
        ...
        hdr_cells = table.rows[0].cells
        hdr_cells[0].text = experience['desc']
